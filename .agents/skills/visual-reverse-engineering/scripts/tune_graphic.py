#!/usr/bin/env python3
"""
Automated vector graphic parameter tuner and multi-objective loss optimizer.
...
Uses component-separated loss (Core SSIM, Sobel edge contour IoU, diffuse glow RMSE, color RMSE)
to prevent diffuse halos from masking fine geometric shape discrepancies.
"""
...
class GraphicTuner:
    ...
    def __init__(
        ...
        self.ref_arr = np.array(reference_arr, dtype=np.float32)
        # Normalize zero-alpha RGB channels
        self.ref_arr[self.ref_arr[:, :, 3] == 0, :3] = 0.0
        self.width = width
        self.height = height
        self.metric = metric
        ...
        self.ref_alpha_mask = self.ref_arr[:, :, 3] > alpha_threshold
        self.ref_core_mask = (self.ref_arr[:, :, 3] > 180) & (np.mean(self.ref_arr[:, :, :3], axis=2) > 180)
        self.ref_glow_mask = (self.ref_arr[:, :, 3] > alpha_threshold) & (~self.ref_core_mask)

    def calculate_loss(self, cand_arr: np.ndarray) -> float:
        """Computes loss between candidate and reference."""
        # Normalize zero-alpha RGB channels
        cand_arr = np.array(cand_arr, dtype=np.float32)
        cand_arr[cand_arr[:, :, 3] == 0, :3] = 0.0

        if self.metric == "rmse":
            diff = self.ref_arr - cand_arr
            return float(math.sqrt(np.mean(diff ** 2)))

        mask_ref = self.ref_alpha_mask
        mask_cand = cand_arr[:, :, 3] > self.alpha_threshold
        inter = np.logical_and(mask_ref, mask_cand).sum()
        union = np.logical_or(mask_ref, mask_cand).sum()
        alpha_iou = float(inter / union) if union > 0 else 0.0

        if self.metric == "combined":
            diff = self.ref_arr - cand_arr
            rmse = float(math.sqrt(np.mean(diff ** 2)))
            return rmse + 60.0 * (1.0 - alpha_iou)

        # Multi-objective component-separated loss
        # 1. Core SSIM
        cand_core_mask = (cand_arr[:, :, 3] > 180) & (np.mean(cand_arr[:, :, :3], axis=2) > 180)
        combined_core_mask = np.logical_or(self.ref_core_mask, cand_core_mask)
        core_ssim = compute_ssim(self.ref_arr, cand_arr, mask=combined_core_mask) if np.any(combined_core_mask) else 1.0

        if self.metric == "ssim":
            return float(1.0 - core_ssim)

        # 2. Sobel Edge Contour IoU
        edge_iou = compute_edge_iou(self.ref_arr, cand_arr)

        # 3. Overlap Color RMSE
        if inter > 0:
            overlap_ref = self.ref_arr[:, :, :3][np.logical_and(mask_ref, mask_cand)]
            overlap_cand = cand_arr[:, :, :3][np.logical_and(mask_ref, mask_cand)]
            color_rmse = float(math.sqrt(np.mean((overlap_ref - overlap_cand) ** 2)))
        else:
            color_rmse = 255.0

        # 4. Diffuse Glow RMSE
        cand_glow_mask = (cand_arr[:, :, 3] > self.alpha_threshold) & (~cand_core_mask)
        combined_glow_mask = np.logical_or(self.ref_glow_mask, cand_glow_mask)
        if np.any(combined_glow_mask):
            glow_diff = self.ref_arr[:, :, :][combined_glow_mask] - cand_arr[:, :, :][combined_glow_mask]
            glow_rmse = float(math.sqrt(np.mean(glow_diff ** 2)))
        else:
            glow_rmse = 0.0

        # Weighted loss composition
        loss = (
            50.0 * (1.0 - core_ssim)
            + 40.0 * (1.0 - edge_iou)
            + 30.0 * (1.0 - alpha_iou)
...
{svg_content}
        ...
        # Fast domcontentloaded avoids 500ms networkidle latency
...
def run_optimization(
    ...
    res = minimize(
        ...
        x0=x0,
        method="Nelder-Mead",
        options={"maxiter": max_iter, "xatol": 0.2, "fatol": 0.2, "disp": False},
    )
...
def main() -> int:
    ...
    parser.add_argument("--output-diff", default=None, help="Path to save 5-panel visual comparison diagnostic artifact")
    ...
    parser.add_argument("-m", "--metric", choices=["multi", "combined", "rmse", "ssim"], default="multi", help="Loss metric to minimize (default: multi)")
