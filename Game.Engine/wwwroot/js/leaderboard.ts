...
export class Leaderboard {
  update(data, position, fleetID) {
    ...
    if (data.Type == "FFA") {
      ...
      const cyanFlagStatus = document
        .getElementById("ctf_cyan")
        .getElementsByClassName("flag_status")[0];
      const redFlagStatus = document
        .getElementById("ctf_red")
        .getElementsByClassName("flag_status")[0];
      ...
      if (flagStatus.cyan == "Home") {
        ...
        cyanFlagStatus
          .getElementsByClassName("taken")[0]
          .classList.remove("hide");
      }
      ...
      const cyanAngle = Math.atan2(
        cyanFlag.Position.y - position.y,
        cyanFlag.Position.x - position.x,
      );
      const redAngle = Math.atan2(
        redFlag.Position.y - position.y,
        redFlag.Position.x - position.x,
      );
      leaderboard.innerHTML = `<tbody>${outR}</tbody>`;
      leaderboardLeft.innerHTML = `<tbody>${outL}</tbody>`;
      ...
      leaderboardCenter.innerHTML =
        ...
        image(
          `ctf_score_final${cyanScore >= 5 ? "_blue" : redScore >= 5 ? "_red" : ""}`,
        ) +
        `</td>` +
...
function drawLeaderArrow(selfPosition, position) {
  ...
  if (angle > 2 * Math.PI - criticalAngle || angle <= criticalAngle) {
    ...
    leaderArrow.style.top =
      (window.innerHeight - leaderArrow.height) / 2 +
      (window.innerWidth / 2) *
        Math.tan(angle) *
        (1 -
          (leaderArrow.height - 2 * leaderArrowTranslate) /
            window.innerHeight) +
      "px";
    ...
    leaderArrow.style.left =
      (window.innerWidth - leaderArrow.width) / 2 +
      (window.innerHeight / 2 / Math.tan(angle)) *
        (1 -
          (leaderArrow.width - 2 * leaderArrowTranslate) / window.innerWidth) +
      "px";
  } else if (
