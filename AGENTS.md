...
Before writing or modifying any code, consult the [**Codebase Map & Architectural Atlas**](.agents/context/codebase_map.md) to immediately locate target files without blind exploratory searches. Then identify your target area and **read the corresponding rule file FIRST**:

| When working on / modifying... | Target Paths / Globs | Mandatory File to Read FIRST | Key Invariants & Pitfalls to Check |
| :--- | :--- | :--- | :--- |
| **All Subsystems (Fast Lookup)** | `*` | [`.agents/context/codebase_map.md`](.agents/context/codebase_map.md) | • Instant symbol-to-file resolution; bypass blind recursive greps<br>• Respect search exclusions (`reference/`, `bin/`, `obj/`, `dist/`) |
| **Game Server, C# & Physics Engine** | `Game.Engine/**`, `Game.API.*/**`, `Game.Registry/**`, `Game.Util/**` | [`.agents/rules/csharp_standards.md`](.agents/rules/csharp_standards.md)<br>[`.agents/rules/game_engine_standards.md`](.agents/rules/game_engine_standards.md)<br>[`.agents/context/physics_model.md`](.agents/context/physics_model.md) | • Zero-allocation hot paths & GC pause prevention<br>• Fixed-timestep tick loop invariants<br>• Thread safety & spatial partitioning (RBush)<br>• FlatBuffers protocol synchronization |
...
| **Vector Graphics & Asset Engineering** | `Game.Engine/wwwroot/img/**`, `.agents/skills/vector-graphics-engineering/**` | [`.agents/skills/vector-graphics-engineering/SKILL.md`](.agents/skills/vector-graphics-engineering/SKILL.md)<br>[`.agents/rules/design_standards.md`](.agents/rules/design_standards.md) | • Retro-neon vector fidelity & multi-pass glow filters<br>• Alpha transparency & zero black background bounding boxes<br>• Perceptual visual diff verification (RMSE, IoU)<br>• Exact tile count and dimension parity with `textureMap_*.scss` |
| **Visual Reverse-Engineering & Parity** | `reference/space1-original/**`, `.agents/skills/visual-reverse-engineering/**` | [`.agents/skills/visual-reverse-engineering/SKILL.md`](.agents/skills/visual-reverse-engineering/SKILL.md) | • Quantitative human-indistinguishability validation thresholds<br>• Zero-alpha RGB normalization before diffing<br>• Automated font recognition via Chamfer distance<br>• Mandatory verification exit code 0 gate before PR |
| **Robots & AI Behaviors** | `Game.Robots/**` | [`.agents/rules/csharp_standards.md`](.agents/rules/csharp_standards.md)<br>[`.agents/rules/robot_ai_standards.md`](.agents/rules/robot_ai_standards.md) | • Context-steering vector arithmetic<br>• Flocking & fleet avoidance heuristics<br>• Genetic algorithm controller constraints |
...
| **CI & Local Verification** | Monorepo root / CI scripts | [`.agents/rules/ci_standards.md`](.agents/rules/ci_standards.md) | • Multi-toolchain build checks (`dotnet`, `npm`, `python`)<br>• Clean output verification before pushing |
...
1. **Gate 1: Rule & Contract Intake (MANDATORY)**:
   - **Step 1 (Atlas First)**: Check [`.agents/context/codebase_map.md`](.agents/context/codebase_map.md) to identify target files and symbols directly, bypassing blind searches.
   - **Step 2 (Search Boundaries)**: Never run unbounded grep or directory searches across `reference/`, `bin/`, `obj/`, or `dist/`. For ambiguous discoveries, invoke the `research` subagent to preserve the primary context window.
   - **Step 3 (Rule Intake)**: Read the required rule files from the *Rule Routing Matrix* using `view_file`.
   - **Step 4 (Inspect Contracts)**: Inspect existing game hooks, models, or telemetry formats before altering logic.
...
3. **Gate 3: Local CI Verification**:
   - Execute all local verification commands (`dotnet build Game.Engine.sln`, client format & build, and `python analysis/verify_codebase_map.py`).
4. **Gate 4: Context Self-Maintenance**:
   ...
   - **Mandatory Invariant**: Whenever adding, moving, deleting, or substantially refactoring key classes, files, or subsystems, you MUST update [`.agents/context/codebase_map.md`](.agents/context/codebase_map.md) in the same PR.
...
| **Verify Codebase Map Links** | Monorepo root | `python analysis/verify_codebase_map.py` |
...
- `.agents/`: Agent configuration schema, rules, architectural context, and maintenance skills
  ...
  - `skills/`: Project-specific skills (`git-pr-workflow`, `agent-maintenance`, `vector-graphics-engineering`, `visual-reverse-engineering`)
